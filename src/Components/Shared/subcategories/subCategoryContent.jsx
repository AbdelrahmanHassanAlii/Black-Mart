    ...
    if (products.length > 0) {
      ...
      if (localStorageFilters.color) {
        filtered = filtered.filter((product) =>
          product.color.includes(localStorageFilters.color)
        );
      }
      if (localStorageFilters.type) {
        filtered = filtered.filter(
          (product) => product.typeof === localStorageFilters.type
        );
      }
      if (localStorageFilters.size) {
        filtered = filtered.filter((product) =>
          product.size.includes(localStorageFilters.size)
        );
      }
      if (localStorageFilters.style) {
        filtered = filtered.filter(
          (product) => product.style === localStorageFilters.style
        );
      }
