...
export default function Prof({ name, email, role, flag }) {
    ...
    <div className={`bg-white p-16 rounded-lg w-96 mb-6 ${flag ? "hidden" : "block"}`}>
      ...
      <div className="space-y-4 h-3/4">
        <div>
          ...
          <input type="text" name="userName" value={userInfo.userName} className="mt-1 block w-full text-xl font-bold border p-2 rounded-xl" onChange={handleChange} />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700">Email</label>
          <input type="text" name="userEmail" value={userInfo.userEmail} className="mt-1 block w-full text-xl font-bold border p-2 rounded-xl" onChange={handleChange} />
        </div>
        <div>
      ...
      <div className="flex gap-5 w-full">
        <button onClick={handleUpdate} className={`bg-blue-100 text-white py-1 px-9 rounded-3xl text-xl hover:bg-blue-400 cursor-pointer ${isModified ? "block" : "hidden"}`}>Update</button>
